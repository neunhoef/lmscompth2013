...
\begin{frame}
$X$: finite system of \Struc{generators}
with involution $X \to X, x \mapsto x^{-1}$

\smallskip
\pause
$X^*$: set of \Alert{finite words} in $X$, concatenation as product

\smallskip
\pause
$R \subseteq X^*$: finite set of \Struc{relations}

\medskip
\pause
\Struc{Define} $\sim$ as  \Alert{finest equivalence relation} on $X^*$ with
\begin{itemize}
\item $uv \sim uxx^{-1}v$ for all $u,v \in X^*$ and all $x \in X$, and
\item $uv \sim urv$ for all $u,v \in X^*$ and all $r \in R$.
\end{itemize}

\pause
\begin{defprop}[Finitely presented group]
The set $G := X^*/\sim$ of equivalence classes is a group with
...
\pause
$\mybaremp:X^* \to G, w \mapsto \mybar{w}$ is the natural map from $w$
...
If $H$ is any group and $f : X \to H$ \Struc{any map} such that for all $r =
x_1 x_2 \cdots x_k \in R$ we have
...
For $G := \left< X \mid R \right>$ and a \Struc{word $w \in X^*$},
...
For $G := \left< X \mid R \right>$ and two \Struc{words $v,w \in X^*$},
\Alert{decide in finitely many steps} whether or not there is a word
$u \in X^*$ such that $\mybar w = \mybar{u v u^{-1}} \in G$.
\end{problem}
...
\pause or even: for a map $f : X \to Y^*$, \Alert{decide in finitely
...
\pause
\begin{lemma}
Let $G=\left< X \mid R\right>$, then $\mybar{w}=1$ for a $w \in X^*$
iff
...
\vspace*{-1.5mm}
for a $k \in \N$ and some $w_i \in X^*$ and $r_i \in R$. 
...
\[ \delta(\ell) := \max\{ A(w) \mid w \in X^* \mbox{ of length } \ell 
\mbox{ and } \mybar w = 1\} \]
\end{definition}
...
iff the Dehn function $\delta$ is \Alert{recursively computable}.
\end{theorem}
